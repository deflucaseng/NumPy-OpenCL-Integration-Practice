...
static PyObject* add_matrix(PyObject* self, PyObject* args) {
    ...
    // Check array types
    if (PyArray_TYPE(input_array1) != NPY_DOUBLE || PyArray_TYPE(input_array2) != NPY_DOUBLE) {
    ...
    PyArrayObject* input_array1_cont = (PyArrayObject*)PyArray_ContiguousFromAny(
        (PyObject*)input_array1, NPY_DOUBLE, 0, 0);
    PyArrayObject* input_array2_cont = (PyArrayObject*)PyArray_ContiguousFromAny(
        (PyObject*)input_array2, NPY_DOUBLE, 0, 0);
    ...
    // Create output array
    PyArrayObject *output_array = (PyArrayObject*)PyArray_SimpleNew(nd1, dims1, NPY_DOUBLE);
    ...
    // Get data pointers
    double *input_data1 = (double*)PyArray_DATA(input_array1_cont);
    double *input_data2 = (double*)PyArray_DATA(input_array2_cont);
    double *output_data = (double*)PyArray_DATA(output_array);
    
    // Call matrix addition
    int err = 0;
    matrix_add(input_data1, dims1[1], dims1[0], input_data2, dims2[1], dims2[0], output_data, &err);
